...
class EnhancedCurriculumMatcher:
    ...
    def __init__(self, log_callback=print):
        ...
        # Add new catalog columns to fetch
        self.CATALOG_SUPPLIER_NAME = "supplier_name"
        self.CATALOG_COPYRIGHT_YEAR = "copyright_year"
...
class MatcherApp:
    ...
    def _run_headless_mode(self):
        input_file_path = "/tbl_MatchedDataSample10000.csv"
        catalog_file_path = "/CEMD_ProdCat_07122025_p.csv"
        output_dir_path = "/content/results/"
        print("--- RUNNING IN HEADLESS MODE ---")
        if not os.path.exists(output_dir_path):
            os.makedirs(output_dir_path)
        # --- REVISED: No longer passes mode ---
        self._run_processing_logic(input_file_path, catalog_file_path, output_dir_path)

    def _run_processing_logic(self, input_path, catalog_path, output_dir):
        ...
        try:
            ...
            for idx, row in input_df.iterrows():
                ...
                # --- 1. Smart QA Scoring (Row-by-Row Detection) ---
                ...
                if pd.notna(human_match_id) and human_match_id in catalog_lookup.index:
                    ...
                    # Add scores with a 'human_match_' prefix
                    for key, val in qa_scores.items():
                        result_row[f"human_match_{key}"] = val

                # --- 2. Always run Automated Matcher ---
                ...
                if not primary_candidates.empty:
                    ...
                    # Find Top 10 to ensure a good pool for ranking
                ...
                for _, cand in top_candidates_df.iterrows():
                    ...
                    # Add additional catalog columns
                    scores[self.matcher.CATALOG_PRODUCT_NAME] = cand.get(
                        self.matcher.CATALOG_PRODUCT_NAME, ""
                    )
                    scores[self.matcher.CATALOG_SERIES] = cand.get(
                        self.matcher.CATALOG_SERIES, ""
                    )
                    scores[self.matcher.CATALOG_SUPPLIER_NAME] = cand.get(
                        self.matcher.CATALOG_SUPPLIER_NAME, ""
                    )
                    scores[self.matcher.CATALOG_COPYRIGHT_YEAR] = cand.get(
                        self.matcher.CATALOG_COPYRIGHT_YEAR, ""
                    )
                    scores[self.matcher.CATALOG_GRADES] = cand.get(
                ...
                # Format output for Top 3 matches
                for i in range(3):
                    if i < len(scored_candidates):
                        match = scored_candidates[i]
                        result_row[f"match_{i+1}_id"] = match[self.matcher.CATALOG_ID]
                        for key, val in match.items():
                            if key not in [
                                self.matcher.CATALOG_ID,
                                self.matcher.CATALOG_PRODUCT_NAME,
                                self.matcher.CATALOG_SERIES,
                                self.matcher.CATALOG_SUPPLIER_NAME,
                                self.matcher.CATALOG_COPYRIGHT_YEAR,
                        ...
                        # Add the new catalog columns to the output row
                        result_row[
                            f"match_{i+1}_{self.matcher.CATALOG_PRODUCT_NAME}"
                        ] = match[self.matcher.CATALOG_PRODUCT_NAME]
                        result_row[f"match_{i+1}_{self.matcher.CATALOG_SERIES}"] = (
                            match[self.matcher.CATALOG_SERIES]
                        )
                        result_row[
                            f"match_{i+1}_{self.matcher.CATALOG_SUPPLIER_NAME}"
                        ] = match[self.matcher.CATALOG_SUPPLIER_NAME]
                        result_row[
                            f"match_{i+1}_{self.matcher.CATALOG_COPYRIGHT_YEAR}"
                        ] = match[self.matcher.CATALOG_COPYRIGHT_YEAR]
                        result_row[f"match_{i+1}_{self.matcher.CATALOG_GRADES}"] = (
                            match[self.matcher.CATALOG_GRADES]
                        )

                    else:
                        result_row[f"match_{i+1}_id"] = ""
                        result_row[f"match_{i+1}_final_score"] = ""
                        result_row[f"match_{i+1}_name_score"] = ""
                        result_row[f"match_{i+1}_publisher_score"] = ""
                        result_row[f"match_{i+1}_grade_score"] = ""
                        result_row[
                            f"match_{i+1}_{self.matcher.CATALOG_PRODUCT_NAME}"
                        ] = ""
                        result_row[f"match_{i+1}_{self.matcher.CATALOG_SERIES}"] = ""
...
# Modified to directly run in headless mode for Colab
app = MatcherApp(headless=True)
app.run()
