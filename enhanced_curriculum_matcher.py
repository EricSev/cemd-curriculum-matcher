...
class EnhancedCurriculumMatcher:
    def __init__(self, log_callback=print):
        ...
        self.CATALOG_GRADES_2 = "intended_grades2"  # For cleaner output
...
class MatcherApp:
    ...
    def _run_automated_matcher(self, input_df, catalog_df, progress_callback):
        all_results = []
        # Create a lookup for enriching the final output
        catalog_lookup = catalog_df.set_index(self.matcher.CATALOG_ID)
        ...
        for idx, row in input_df.iterrows():
            ...
            result_row = row.to_dict()
            # --- CHANGED: Loop through Top 5 to create output columns ---
            for i in range(5):
                if i < len(scored_candidates):
                    ...
                    # --- NEW: Add enriched catalog data columns ---
                    if match_id in catalog_lookup.index:
                        ...
                        # Use cleaner intended_grades2 if it exists, otherwise fall back
                        grades_col = (
